...
export default function ACProductDetail() {
  const router = useRouter();
  const params = useParams();
  const id = (params.id as string)?.toUpperCase();

  const product = acProducts.find((p) => p.id.toUpperCase() === id) || acProducts[0];

  const specs = (product as any).specs || {};
  const included = (product as any).included || {};
  const features = (product as any).features || ["Diseño compacto y ligero", "Mango ergonómico", "Rendimiento profesional"];
              ...
              {(product as any).topBullets ? (
                ...
                <>
                  <li className="font-inter text-[14px] font-[300] leading-[1.4] text-black">
                    &#x2022; Control de potencia continua
                  </li>
                  <li className="font-inter text-[14px] font-[300] leading-[1.4] text-black">
                    &#x2022; Diseño robusto para uso profesional
                  </li>
                </>
              )}
            </ul>
